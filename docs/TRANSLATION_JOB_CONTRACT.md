...
Production configuration version 7 limits a model batch to 1,200 source characters, includes only glossary entries and protected names relevant to that batch, accepts the model's validated bare-array response, and applies inflection-aware terminology checks. Smaller bounded batches avoid the nonlinear runtime and memory growth seen with full-article prompts while preserving segment order. It provides a short Spanish-to-English JSON demonstration before the article payload so the model follows the translation task instead of copying a larger Spanish batch. If one translated segment fails validation, the worker makes one focused correction attempt for that segment and validates it again; it still publishes nothing unless every segment passes.
