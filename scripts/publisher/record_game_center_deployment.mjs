...
if (result === "success") {
  ...
  await atomicJson(statePath, { ...state, pendingJobId: null, pendingSince: null, lastDeploymentAt: new Date().toISOString(), lastDeploymentJobId: jobId, consecutiveFailures: 0, backoffUntil: null });
  await createNotification(join(publisherRoot, "notifications"), { level: "success", event: "game-center-updated", message: "Game Center se validó y se publicó con autorización manual.", replacePending: true });
} else {
  await atomicJson(statePath, { ...state, pendingJobId: null, pendingSince: null, lastFailureAt: new Date().toISOString(), backoffUntil: new Date(Date.now() + 60 * 60_000).toISOString() });
  await createNotification(join(publisherRoot, "notifications"), { level: "error", event: "game-center-update-failed", message: "La publicación manual de Game Center falló. Producción fue conservada o restaurada automáticamente.", replacePending: true });
}
