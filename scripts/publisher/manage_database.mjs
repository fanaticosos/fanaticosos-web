...
import { applyMusicImport, previewMusicImport } from "../../publisher/lib/music-import.mjs";
...
async function main(argv) {
  ...
  } else if (command === "audio-import-apply") {
    result = await applyAudioImport({
      databasePath: required(parsed, "--database"),
      statesRoot: required(parsed, "--states-root"),
      jobsRoot: required(parsed, "--jobs-root"),
      artifactsRoot: required(parsed, "--artifacts-root"),
    });
  } else if (command === "release-import-preview") {
    result = await previewReleaseImport({
      databasePath: required(parsed, "--database"),
      statesRoot: required(parsed, "--states-root"),
      releasesRoot: required(parsed, "--releases-root"),
    });
  } else if (command === "release-import-apply") {
    result = await applyReleaseImport({
      databasePath: required(parsed, "--database"),
      statesRoot: required(parsed, "--states-root"),
      releasesRoot: required(parsed, "--releases-root"),
      imagesRoot: required(parsed, "--images-root"),
    });
  } else if (command === "audiogram-import-preview") {
    result = await previewAudiogramImport({
      databasePath: required(parsed, "--database"),
      statesRoot: required(parsed, "--states-root"),
      jobsRoot: required(parsed, "--jobs-root"),
    });
  ...
  } else if (command === "music-import-apply") {
    result = await applyMusicImport({ databasePath: required(parsed, "--database"), settingsPath: required(parsed, "--settings"),
      statesRoot: required(parsed, "--states-root"), releasesRoot: required(parsed, "--releases-root") });
  } else {
