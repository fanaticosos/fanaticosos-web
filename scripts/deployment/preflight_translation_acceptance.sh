...
if ! sudo grep -Fq -- '--configuration-version 4' "$source_unit"; then
  echo "STOP: Versioned unit is not configuration version 4."
  exit 1
fi
...
if sudo cmp -s "$source_unit" "$installed_unit"; then
  echo "Installed template already matches configuration version 4."
else
  echo "CHANGE REQUIRED: Installed template must be replaced with version 4."
fi
...
echo "NEXT: Install the verified version 4 template and create job $job_id."
