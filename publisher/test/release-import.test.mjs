...
import { applyReleaseImport, previewReleaseImport } from "../lib/release-import.mjs";
...
test("release import preview excludes fixtures and verifies the published catalog and assets", async () => {
  ...
  await writeFile(join(statesRoot, `deployment-${draft.articleId}.json`), JSON.stringify({ schemaVersion: 1, articleId: draft.articleId, draftRevision: 1, releaseJobId: jobId, status: "completed", createdAt: "2026-09-09T01:02:00Z", updatedAt: "2026-09-09T01:03:00Z", receipt: { url: "https://fixture-id.example.pages.dev", validatedAt: "2026-09-09T01:03:00Z" } }));
  ...
  const applied = await applyReleaseImport({ statesRoot, releasesRoot, imagesRoot: join(root, "image-artifacts"), databasePath });
  assert.equal(applied.releases, 1); assert.equal(applied.deployments, 1);
  const inspected = await openDatabase(databasePath);
  assert.equal(inspected.prepare("SELECT COUNT(*) AS count FROM releases WHERE status = 'validated'").get().count, 1);
  assert.equal(inspected.prepare("SELECT COUNT(*) AS count FROM article_catalog_entries").get().count, 1);
  assert.equal(inspected.prepare("SELECT COUNT(*) AS count FROM release_artifacts").get().count, 3);
  assert.equal(inspected.prepare("SELECT status FROM deployments").get().status, "published");
  closeDatabase(inspected);
  const repeated = await applyReleaseImport({ statesRoot, releasesRoot, imagesRoot: join(root, "image-artifacts"), databasePath });
  assert.equal(repeated.insert, 0); assert.equal(repeated.unchanged, 1);
});
