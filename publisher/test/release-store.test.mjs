...
test("SQLite release store owns state while the filesystem carries only worker requests and manifests", async () => {
  ...
  try {
    ...
    const store = databaseReleaseStore({ database, queueRoot, releasesRoot });
    ...
    assert.equal(database.prepare("SELECT COUNT(*) AS count FROM release_artifacts WHERE release_id = ?").get(queued.jobId).count, 3);
